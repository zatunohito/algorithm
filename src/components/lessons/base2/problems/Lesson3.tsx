    ...
    <>
      ...
      <div className="p-6 bg-gray-900/60 rounded-lg border border-gray-800">
        ...
        <details className="mt-4 group">
          ...
          <div className="mt-2 pt-4 border-t border-gray-700 text-gray-300">
            <p><strong>答え:</strong> `[3, 5, 1, 4, 8]`</p>
            <p className="mt-2"><strong>理由:</strong> バブルソートの1回のパスでは、隣り合う要素を比較し、左が大きい場合に交換します。この操作を配列の末尾まで繰り返すと、最も大きい要素が右端に移動します。</p>
      ...
      <div className="p-6 bg-gray-900/60 rounded-lg border border-gray-800">
        ...
        <details className="mt-4 group">
          <summary className="cursor-pointer text-blue-400 hover:text-blue-300 list-none group-open:text-white">答えを見る</summary>
          <div className="mt-2 pt-4 border-t border-gray-700 text-gray-300">
            <p><strong>答え:</strong> O(n²)</p>
            <p className="mt-2"><strong>理由:</strong> バブルソートは二重ループ構造を持ち、外側のループと内側のループがそれぞれ約n回実行されます。そのため、比較・交換の回数はnの2乗に比例し、計算量はO(n²)となります。これは、データが完全に逆順にソートされている場合などに相当します。</p>
          </div>
        </details>
      </div>
      ...
      <div className="p-6 bg-gray-900/60 rounded-lg border border-gray-800">
        ...
        <details className="mt-4 group">
          <summary className="cursor-pointer text-blue-400 hover:text-blue-300 list-none group-open:text-white">答えを見る</summary>
          <div className="mt-2 pt-4 border-t border-gray-700 text-gray-300">
            <p><strong>答え:</strong> `n - i - 2`</p>
            <p className="mt-2"><strong>理由:</strong> 外側のループ `i` が1回完了するごとに、配列の右端から `i` 個の要素のソートが確定します。したがって、内側のループで比較する必要があるのは `n - i` 個の要素です。この範囲で隣り合う要素 `data[j]` と `data[j+1]` を比較するため、`j` のインデックスは `0` から `(n - i - 1) - 1`、つまり `n - i - 2` までとなります。</p>
          </div>
        </details>
      </div>
      ...
      <div className="p-6 bg-gray-900/60 rounded-lg border border-gray-800">
        ...
        <details className="mt-4 group">
          <summary className="cursor-pointer text-blue-400 hover:text-blue-300 list-none group-open:text-white">答えを見る</summary>
          <div className="mt-2 pt-4 border-t border-gray-700 text-gray-300">
            <p><strong>答え:</strong> 4回</p>
            <p className="mt-2"><strong>理由:</strong> バブルソートでは、要素数が `n` の場合、最大で `n-1` 回のパスが必要です。1回のパスで少なくとも1つの要素（その時点で最大の要素）が正しい位置に確定するためです。要素数が5の場合、`5 - 1 = 4` 回のパスで全ての要素が整列されます。</p>
          </div>
        </details>
      </div>
    </>
